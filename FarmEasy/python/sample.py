...
class sample(object):
    ...
    def analyze(self, temp, stateName, countyName, cropName):
        ...
        sales_irr_str = cropName + ", IRRIGATED - SALES, MEASURED IN $"
        ...
        sales_measured_irr = temp[ (temp.commodity == cropName) & (temp.state == stateName) & (temp.county == countyName) & (temp.data_item == sales_irr_str)]
        ...
        sales_nirr_str = cropName + ", NON-IRRIGATED - SALES, MEASURED IN $"
        ...
        sales_measured_nirr = temp[ (temp.commodity == cropName) & (temp.state == stateName) & (temp.county == countyName) & (temp.data_item == sales_nirr_str)]
        list = {}
        ...
        list.update({'sales_irr': sales_measured_irr.value.mean().round(2)})
        ...
        list.update({'sales_nirr': sales_measured_nirr.value.mean().round(2)})
